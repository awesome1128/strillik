...
class Opal_WP_Template extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            'opal-wp_template',
            esc_html__( 'Opal Elementor Template', 'strollik5-core' ),
            [
                'description' => esc_html__( 'Embed your saved elements.', 'strollik5-core' ),
            ]
        );
    }
    ...
    public function form( $instance ) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title', 'strollik5-core' ); ?>:</label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>">
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'template_id' ) ); ?>"><?php esc_attr_e( 'Choose Template', 'strollik5-core' ); ?>:</label>
            <select class="widefat elementor-widget-template-select" id="<?php echo esc_attr( $this->get_field_id( 'template_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'template_id' ) ); ?>">
                <option value="">— <?php _e( 'Select', 'strollik5-core' ); ?> —</option>
                <?php
                foreach ( $templates as $template ) :
            ...
            <a target="_blank" class="elementor-edit-template"<?php echo $style; ?> href="<?php echo add_query_arg( 'elementor', '', get_permalink( $instance['template_id'] ) ); ?>">
                <i class="fa fa-pencil"></i> <?php echo __( 'Edit Template', 'strollik5-core' ); ?>
            </a>
        </p>
        <?php
    }
