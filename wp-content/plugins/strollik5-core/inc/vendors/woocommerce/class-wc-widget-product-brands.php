...
class OSF_Widget_Product_Brands extends WC_Widget{
    ...
    public function __construct() {
        ...
        $this->widget_description = __( 'A list or dropdown of product brands.', 'strollik5-core' );
        $this->widget_id          = 'woocommerce_product_brands';
        $this->widget_name        = __( 'Product Brands', 'strollik5-core' );
        $this->settings           = array(
            'title'  => array(
                'type'  => 'text',
                'std'   => __( 'Product Brands', 'strollik5-core' ),
                'label' => __( 'Title', 'strollik5-core' ),
            ),
            'dropdown' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show as dropdown', 'strollik5-core' ),
            ),
            'show_logo' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show logo brand', 'strollik5-core' ),
            ),
            'count' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show product counts', 'strollik5-core' ),
            ),
            'hierarchical' => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => __( 'Show hierarchy', 'strollik5-core' ),
            ),
            'show_children_only' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Only show children of the current brand', 'strollik5-core' ),
            ),
            'hide_empty' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Hide empty brands', 'strollik5-core' ),
            ),
            'max_depth'  => array(
                'type'  => 'text',
                'std'   => '',
                'label' => __( 'Maximum depth', 'strollik5-core' ),
            ),
        );
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $dropdown ) {
            ...
            include_once( STROLLIK5_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-product-brand-list-walker.php' );

            $list_args['walker']                     = new OTF_Product_Brand_List_Walker;
            $list_args['title_li']                   = '';
            $list_args['pad_counts']                 = 1;
            $list_args['show_option_none']           = __( 'No product brands exist.', 'strollik5-core' );
            ...
            echo '<input type="text" id="otf-product-brands-'. $id .'" onkeyup="Otf_search_product_brand();" placeholder="'. esc_attr__('Search for brand name...', 'strollik5-core').'" title="'.esc_attr__('Type in a name','strollik5-core').'">';
