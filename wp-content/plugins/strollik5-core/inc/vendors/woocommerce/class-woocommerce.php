...
//            add_action('wp_ajax_nopriv_osf_ajax_loadmore_products', array($this, 'products_load_more'));
		...
		public function mobile_handheld_footer_bar() {
			...
			echo '';
			$links = array(
				'my-account' => '<a class="my-accrount-footer" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'My Account', 'strollik5-core' ) . '</a>',
				'search'     => '<a class="search-footer" href="">' . esc_attr__( 'Search', 'strollik5-core' ) . '</a><div class="site-search">' . get_search_form( false ) . '</div>',
				'cart'       => ' <a class="footer-cart-contents" href="' . esc_url( wc_get_cart_url() ) . '" 
                title="' . esc_attr__( 'View your shopping cart', 'strollik5-core' ) . '">  
 <span class="count">' . wp_kses_data( WC()->cart->get_cart_contents_count() ) . '</span>
		...
		public function label_tooltip() {
			...
			echo '<div id="osf-woocommerce-cart">' . esc_html__( 'Add to cart', 'strollik5-core' ) . '</div>';
			echo '</div>';
		}
		...
		public function added_to_cart_template() {
			$text = esc_html__( 'has been added to your cart', 'strollik5-core' );
			echo <<<HTML
...
//	    }
		...
		public function widgets_init() {
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop', 'strollik5-core' ),
				'id'            => 'sidebar-woocommerce-shop',
				'description'   => __( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'strollik5-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Detail', 'strollik5-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop Filters', 'strollik5-core' ),
		...
		public function render_woocommerce_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop-filters' ) ) {
				echo '<div id="opal-canvas-filter" class="opal-canvas-filter ' . $position . '"><span class="filter-close">' . esc_html__( 'CLOSE', 'strollik5-core' ) . '</span><div class="opal-canvas-filter-wrap">';
		...
		public function render_button_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop-filters' ) ) {
				echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__( 'Filter', 'strollik5-core' ) . '</button>';
			}
		}
			...
			if ( ! empty( $query ) ) {
				...
				if ( is_object( $product_object ) ) {
					...
					if ( ! empty( $product_sku ) ) {
						$product_sku_display = ' - ' . __( 'Sku', 'strollik5-core' ) . ': ' . $product_sku;
					}

					$product_title_display = '';
					if ( ! empty( $product_title ) ) {
						$product_title_display = ' - ' . __( 'Title', 'strollik5-core' ) . ': ' . $product_title;
					}

					$product_id_display = __( 'Id', 'strollik5-core' ) . ': ' . $product_id;
		...
		public function productCategoryAutocompleteRender( $query ) {
			...
			$term_slug_display = '';
			if ( ! empty( $term_slug ) ) {
				$term_slug_display = ' - ' . __( 'Sku', 'strollik5-core' ) . ': ' . $term_slug;
			}

			$term_title_display = '';
			if ( ! empty( $term_title ) ) {
				$term_title_display = ' - ' . __( 'Title', 'strollik5-core' ) . ': ' . $term_title;
			}

			$term_id_display = __( 'Id', 'strollik5-core' ) . ': ' . $term_id;
		...
		public function taxonomy_metaboxes() {
			...
			$cmb_term = new_cmb2_box( array(
				'id'           => 'product_cat',
				'title'        => __( 'Product Metabox', 'strollik5-core' ), // Doesn't output for term boxes
		...
		public function product_video_custom_field() {
			...
			$cmb    = new_cmb2_box( array(
				...
				'title'        => esc_html__( 'Product Video Config', 'strollik5-core' ),
			...
			$cmb->add_field( array(
				'name' => __( 'Product video', 'strollik5-core' ),
				'desc' => __( 'Supports video from youtube and vimeo.', 'strollik5-core' ),
				'id'   => $prefix . 'video',
				'type' => 'oembed',
			) );

			$cmb->add_field( array(
				'name'         => __( 'Video Thumbnail', 'strollik5-core' ),
		...
		public function woocommerce_scripts() {
			...
			wp_enqueue_script( 'osf-woocommerce-main', trailingslashit( STROLLIK5_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/main.js', array(
				'jquery',
				'wp-util'
			), false, true );

			if ( is_product() ) {
				wp_enqueue_script( 'osf-woocommerce-single', trailingslashit( STROLLIK5_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/single.js', array(), false, true );
				if ( get_theme_mod( 'osf_woocommerce_single_product_tab_style', 'tab' ) == 'accordion' ) {
					wp_enqueue_script( 'osf-woocommerce-single-accordion', trailingslashit( STROLLIK5_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/woocommerce-accordion.js', array(), false, true );
				}
			}
