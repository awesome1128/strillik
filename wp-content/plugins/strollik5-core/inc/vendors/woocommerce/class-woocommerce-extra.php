...
class Opal_Woocommerce_Extra {
    ...
    public function __construct() {
        ...
        include_once(STROLLIK5_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-wc-widget-product-brands.php');
        include_once(STROLLIK5_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-wc-widget-layered-nav.php');
    ...
    public function enqueue_scripts($hook) {
        if ($hook === 'post.php' && get_post_type() === 'product') {
            wp_enqueue_script('otf-woocommerce-admin', STROLLIK5_CORE_PLUGIN_URL . 'assets/js/woocommerce/admin.js', array('jquery'), STROLLIK5_CORE_VERSION, true);
        }
    }
    ...
    public function custom_taxonomy_brands() {
        ...
        $labels = array(
            'name'                       => esc_html__('Brands', 'strollik5-core'),
            'singular_name'              => esc_html__('Brands', 'strollik5-core'),
            'menu_name'                  => esc_html__('Brands', 'strollik5-core'),
            'all_items'                  => esc_html__('All Brands', 'strollik5-core'),
            'parent_item'                => esc_html__('Parent Brand', 'strollik5-core'),
            'parent_item_colon'          => esc_html__('Parent Brand:', 'strollik5-core'),
            'new_item_name'              => esc_html__('New Brand Name', 'strollik5-core'),
            'add_new_item'               => esc_html__('Add New Brands', 'strollik5-core'),
            'edit_item'                  => esc_html__('Edit Brand', 'strollik5-core'),
            'update_item'                => esc_html__('Update Brand', 'strollik5-core'),
            'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'strollik5-core'),
            'search_items'               => esc_html__('Search Brands', 'strollik5-core'),
            'add_or_remove_items'        => esc_html__('Add or remove Brands', 'strollik5-core'),
            'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'strollik5-core'),
        );
        $args   = array(
            'labels'            => $labels,
            'hierarchical'      => true,
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'strollik5-core') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'strollik5-core')); ?>
            <input type="number" min="<?php echo $min; ?>" class="short" name="_deal_quantity" id="_deal_quantity" value="<?php echo esc_attr($quantity) ?>">

            <?php
            if ($quantity > 0) {
                echo '<span class="deal-sold-counts" style="clear:both;display:block;"><strong>' . sprintf(_n('%s product is sold', '%s products are sold', max(1, $sales_counts), 'bicomart-core', 'filaminimal-core', 'strollik5-core', 'strollik5-core'), $sales_counts) . '</strong></span>';
            }
            ?>
        </p>
