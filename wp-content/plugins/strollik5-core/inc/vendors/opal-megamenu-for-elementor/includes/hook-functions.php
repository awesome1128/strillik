...
function osf_add_back_button_inspector() {
	...
	?>
        ...
        <script type="text/template" id="tmpl-elementor-panel-footer-content">
            <div id="elementor-panel-footer-back-to-admin" class="elementor-panel-footer-tool elementor-leave-open tooltip-target" data-tooltip="<?php esc_attr_e( 'Back', 'strollik5-core' ); ?>">
				<i class="fa fa-arrow-left" aria-hidden="true"></i>
			</div>
			<div id="elementor-panel-footer-responsive" class="elementor-panel-footer-tool">
				<i class="eicon-device-desktop tooltip-target" aria-hidden="true" data-tooltip="<?php esc_attr_e( 'Responsive Mode', 'strollik5-core' ); ?>"></i>
				<span class="elementor-screen-only">
					<?php echo __( 'Responsive Mode', 'strollik5-core' ); ?>
				</span>
				<div class="elementor-panel-footer-sub-menu-wrapper">
					<div class="elementor-panel-footer-sub-menu">
						<div class="elementor-panel-footer-sub-menu-item" data-device-mode="desktop">
							...
							<span class="elementor-title"><?php echo __( 'Desktop', 'strollik5-core' ); ?></span>
							<span class="elementor-description"><?php echo __( 'Default Preview', 'strollik5-core' ); ?></span>
						</div>
						<div class="elementor-panel-footer-sub-menu-item" data-device-mode="tablet">
							...
							<span class="elementor-title"><?php echo __( 'Tablet', 'strollik5-core' ); ?></span>
							<?php $breakpoints = Elementor\Responsive::get_breakpoints(); ?>
							<span class="elementor-description"><?php echo sprintf( __( 'Preview for %s', 'strollik5-core' ), $breakpoints['md'] . 'px' ); ?></span>
						</div>
						<div class="elementor-panel-footer-sub-menu-item" data-device-mode="mobile">
							...
							<span class="elementor-title"><?php echo __( 'Mobile', 'strollik5-core' ); ?></span>
							<span class="elementor-description"><?php echo __( 'Preview for 360px', 'strollik5-core' ); ?></span>
						</div>
					</div>
				</div>
			</div>
			<div id="elementor-panel-footer-history" class="elementor-panel-footer-tool elementor-leave-open tooltip-target" data-tooltip="<?php esc_attr_e( 'History', 'strollik5-core' ); ?>">
				<i class="fa fa-history" aria-hidden="true"></i>
				<span class="elementor-screen-only"><?php echo __( 'History', 'strollik5-core' ); ?></span>
			</div>
			<div id="elementor-panel-saver-button-preview" class="elementor-panel-footer-tool tooltip-target" data-tooltip="<?php esc_attr_e( 'Preview Changes', 'strollik5-core' ); ?>">
				<span id="elementor-panel-saver-button-preview-label">
					<i class="fa fa-eye" aria-hidden="true"></i>
					<span class="elementor-screen-only"><?php echo __( 'Preview Changes', 'strollik5-core' ); ?></span>
				</span>
			</div>
			<div id="elementor-panel-saver-publish" class="elementor-panel-footer-tool">
				<button id="elementor-panel-saver-button-publish" class="elementor-button elementor-button-success elementor-saver-disabled">
					...
					<span id="elementor-panel-saver-button-publish-label">
						<?php echo __( 'Publish', 'strollik5-core' ); ?>
					</span>
				</button>
			</div>
			<div id="elementor-panel-saver-save-options" class="elementor-panel-footer-tool" >
				<button id="elementor-panel-saver-button-save-options" class="elementor-button elementor-button-success tooltip-target elementor-saver-disabled" data-tooltip="<?php esc_attr_e( 'Save Options', 'strollik5-core' ); ?>">
					<i class="fa fa-caret-up" aria-hidden="true"></i>
					<span class="elementor-screen-only"><?php echo __( 'Save Options', 'strollik5-core' ); ?></span>
				</button>
				<div class="elementor-panel-footer-sub-menu-wrapper">
					...
					<div class="elementor-panel-footer-sub-menu">
						<div id="elementor-panel-saver-menu-save-draft" class="elementor-panel-footer-sub-menu-item elementor-saver-disabled">
							...
							<span class="elementor-title"><?php echo __( 'Save Draft', 'strollik5-core' ); ?></span>
						</div>
						<div id="elementor-panel-saver-menu-save-template" class="elementor-panel-footer-sub-menu-item">
							...
							<span class="elementor-title"><?php echo __( 'Save as Template', 'strollik5-core' ); ?></span>
						</div>
					</div>
				</div>
			</div>
...
function osf_load_menu_data() {
	...
	if ( ! wp_verify_nonce( $nonce, 'osf-menu-data-nonce' ) || ! $menu_id ) {
		wp_send_json( array(
				'message' => __( 'Access denied', 'strollik5-core' )
			) );
	}
						...
						<div class="osf-modal-content">
							...
							<% } else { %>
								<div class="form-group">
									<label for="icon_color"><?php _e( 'Menu Description', 'strollik5-core' ) ?></label>
									<input type="text" name="opal-menu-item[description]" value="<%= data.description %>" class="input" id="menu_description" />
								</div>
								<div class="form-group">
									<label for="icon"><?php _e( 'Icon', 'strollik5-core' ) ?></label>
								...
								<div class="form-group submenu-setting toggle-select-setting">
									<label><?php _e( 'Mega Submenu Enabled', 'strollik5-core' ) ?></label>
									<select name="opal-menu-item[enabled]" class="opal-input-switcher opal-input-switcher-true" data-target=".submenu-width-setting">
										<option value="1" <%= data.enabled == 1? 'selected':'' %>> <?php _e( 'Yes', 'strollik5-core' ) ?></opttion>
										<option value="0" <%= data.enabled == 0? 'selected':'' %>><?php _e( 'No', 'strollik5-core' ) ?></opttion>
									</select>
									<button id="edit-megamenu" class="button button-primary button-large">
										<?php _e( 'Edit Megamenu Submenu', 'strollik5-core' ) ?>
									</button>
								</div>

								<div class="form-group submenu-width-setting toggle-select-setting" style="display: none">
									<label><?php _e( 'Sub Megamenu Width', 'strollik5-core' ) ?></label>
									<select name="opal-menu-item[customwidth]" class="opal-input-switcher opal-input-switcher-true" data-target=".submenu-subwidth-setting">
										<option value="1" <%= data.customwidth == 1? 'selected':'' %>> <?php _e( 'Yes', 'strollik5-core' ) ?></opttion>
                                        <option value="3" <%= data.customwidth == 3? 'selected':'' %>><?php _e( 'Set Container Width', 'strollik5-core' ) ?></opttion>
										<option value="0" <%= data.customwidth == 0? 'selected':'' %>><?php _e( 'Set Full Width', 'strollik5-core' ) ?></opttion>
										<option value="2" <%= data.customwidth == 2? 'selected':'' %>><?php _e( 'Set Stretch Width', 'strollik5-core' ) ?></opttion>
										<option value="4" <%= data.customwidth == 4? 'selected':'' %>><?php _e( 'Set Left Full Width', 'strollik5-core' ) ?></opttion>
									</select>
								</div>

								<div class="form-group submenu-width-setting submenu-subwidth-setting toggle-select-setting" style="display: none">
									<label for="menu_subwidth"><?php _e( 'Sub Mega Menu Max Width', 'strollik5-core' ) ?></label>
