...
class OSF_Scripts {
    ...
    public function add_frontend_scripts() {
        ...
        wp_register_script('otf-countdown', trailingslashit(STROLLIK5_CORE_PLUGIN_URL) . 'assets/js/countdown.js', array('jquery'), false, true);
        ...
        if (get_theme_support('opal-customize-css')) {
            ...
            if (is_customize_preview() || (osf_is_elementor_activated()  && \Elementor\Plugin::$instance->editor->is_edit_mode() )) {
...
//		wp_enqueue_script( 'imagesLoaded', trailingslashit( STROLLIK5_CORE_PLUGIN_URL ) . 'assets/js/libs/imagesloaded.pkgd.min.js', array( 'jquery' ), '3.0.6' );
//		wp_enqueue_script( 'isotope', trailingslashit( STROLLIK5_CORE_PLUGIN_URL ) . 'assets/js/libs/isotope.pkgd.min.js', array( 'jquery' ), '3.0.6' );

        // Owl Carousel
        wp_enqueue_script('owl-carousel', trailingslashit(STROLLIK5_CORE_PLUGIN_URL) . 'assets/js/libs/owl.carousel.js', array('jquery'), '2.2.1');
        wp_enqueue_script('otf-carousel', trailingslashit(STROLLIK5_CORE_PLUGIN_URL) . 'assets/js/carousel.js', array('owl-carousel'));

        wp_enqueue_script('smoothscroll', trailingslashit(STROLLIK5_CORE_PLUGIN_URL) . 'assets/js/SmoothScroll.min.js', array('jquery'), '1.4.8', true);
    }
    ...
    public function init_panel_customize_js() {
        wp_enqueue_script(
            'otf-admin-customize',
            STROLLIK5_CORE_PLUGIN_URL . 'assets/js/customizer/customize.js',
            array('jquery'),
            null,
            true
        );
    ...
    public function init_preview_js() {
        wp_enqueue_style('otf-theme-preview', STROLLIK5_CORE_PLUGIN_URL . 'assets/css/customize-preview.css');
        wp_enqueue_script('otf-theme-preview', STROLLIK5_CORE_PLUGIN_URL . 'assets/js/customizer/preview.js', array(
            'jquery',
            'customize-preview'
        ), STROLLIK5_CORE_VERSION, true);
    ...
    public function add_admin_scripts($hook) {
        ...
        if ($hook === 'widgets.php') {
            wp_enqueue_script(
                'otf-admin-color',
                STROLLIK5_CORE_PLUGIN_URL . 'assets/3rd-party/alpha-color-picker/alpha-color-picker.js',
                array('jquery',
                      'wp-color-picker'),
                null,
                true
        ...
        wp_enqueue_script(
            'otf-admin-select2',
            STROLLIK5_CORE_PLUGIN_URL . 'assets/3rd-party/select2/select2.js',
            array('jquery'),
            null,
            true
        );
        ...
        wp_enqueue_style('otf-admin-style',
            STROLLIK5_CORE_PLUGIN_URL . 'assets/css/admin.css');

        if ($hook == 'post-new.php' || $hook == 'post.php') {
            wp_enqueue_script('otf-admin-page', trailingslashit(STROLLIK5_CORE_PLUGIN_URL) . 'assets/js/admin-page.js');
        }
