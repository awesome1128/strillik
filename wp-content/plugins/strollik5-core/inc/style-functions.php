...
function osf_theme_custom_css() {
    $a1root = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/a1root.php';
    $grid = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/grid.php';
    $button = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/button.php';
    $error_404 = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/error-404.php';
    $heading = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/heading.php';
    $main_layout = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/main-layout.php';
    $page_bg = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/page-bg.php';
    $page_title = include trailingslashit(STROLLIK5_CORE_PLUGIN_DIR) . 'inc/customize/page-title.php';
    $css = <<<CSS
{$a1root}
{$grid}
{$error_404}
...
function osf_theme_custom_css_no_cache($css) {
    ...
    if ($osf_header) {
        ...
        if(!empty($bg_header)){
            $css .= '@media(max-width: 1024px){.opal-header-absolute .site-header{background:'.$bg_header.';}}';
        }
    }
