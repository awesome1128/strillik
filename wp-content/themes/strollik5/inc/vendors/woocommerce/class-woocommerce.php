...
if (!class_exists('strollik5_WooCommerce')) :


    class strollik5_WooCommerce {
        ...
        /**
         * @return strollik5_WooCommerce
         */
        public static function getInstance() {
            if (!isset(self::$instance) && !(self::$instance instanceof strollik5_WooCommerce)) {
                self::$instance = new strollik5_WooCommerce();
            }
        ...
        public function __construct() {
            ...
            add_filter('strollik5_customizer_buttons', array($this, 'customizer_buttons'));
            ...
            if (class_exists('YITH_WCWL')) {
                add_action('wp_ajax_strollik5_update_wishlist_count', array($this, 'yith_wcwl_ajax_update_count'));
                add_action('wp_ajax_nopriv_strollik5_update_wishlist_count', array($this, 'yith_wcwl_ajax_update_count'));
            }
            ...
            // Woocommerce 3.3
            if (strollik5_woocommerce_version_check('3.3')) {
                add_action('customize_register', array($this, 'edit_section_customizer'), 99);
            }

            // Wocommerce filter
            if (is_active_sidebar('sidebar-woocommerce-shop-filters') && get_theme_mod('strollik5_woocommerce_archive_filter_position', 'left') !== 'none') {
            ...
            if (get_theme_mod('strollik5_woocommerce_archive_filter_position', 'left') == 'top') {
        ...
        public function grouped_product_column_image($grouped_product_child) {
            echo '<td class="woocommerce-grouped-product-image">' . wp_kses_post($grouped_product_child->get_image('thumbnail')) . '</td>';
        }
        ...
        public function mobile_handheld_footer_bar() {
            $links = array(
                'my-account' => '<a class="my-accrount-footer" href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'strollik5') . '</a>',
                'search'     => '<a class="search-footer" href="">' . esc_attr__('Search', 'strollik5') . '</a><div class="site-search">' . get_search_form(false) . '</div>',
                'cart'       => ' <a class="footer-cart-contents" href="' . esc_url(wc_get_cart_url()) . '" 
                title="' . esc_attr__('View your shopping cart', 'strollik5') . '">  
 <span class="count">' . wp_kses_data(WC()->cart->get_cart_contents_count()) . '</span>
                ...
                <ul class="columns-<?php echo count($links); ?>">
                    <?php foreach ($links as $key => $content) : ?>
                        <li class="<?php echo esc_attr($key); ?>">
                            <?php echo apply_filters('strollik5_mobile_handheld_footer_bar', $content); ?>
                        </li>
                    <?php endforeach; ?>
                </ul>
            </div>
            ...
            if (function_exists('wvs_get_wc_attribute_taxonomy')) {
                ...
                switch ($attr->attribute_type) {
                    case 'color':
                        ...
                        $html  = '';
                        $html  .= '<a class="xstreet-color-type" href="' . esc_url($link) . '">';
                    ...
                    case 'button':
                        ...
                        $html  = '';
                        $html  .= '<a class="xstreet-label-type" href="' . esc_url($link) . '">';
                    ...
                    case 'image':
                        ...
                        $html = '';
                        $html .= '<a class="xstreet-image-type" href="' . esc_url($link) . '">';
        ...
        public function register_elementor_wc_hook() {
            ...
            strollik5_include_hooks_product_blocks();
        }
        ...
        public function edit_section_customizer($wp_customizer) {
            $wp_customizer->get_control('woocommerce_single_image_width')->section  = 'strollik5_woocommerce_single';
            $wp_customizer->get_control('woocommerce_single_image_width')->priority = 9;

            $wp_customizer->get_control('woocommerce_thumbnail_image_width')->section = 'strollik5_woocommerce_product';
            $wp_customizer->get_control('woocommerce_thumbnail_cropping')->section    = 'strollik5_woocommerce_product';

            $wp_customizer->get_control('woocommerce_shop_page_display')->section  = 'strollik5_woocommerce_archive';
            $wp_customizer->get_control('woocommerce_shop_page_display')->priority = 21;

            $wp_customizer->get_control('woocommerce_category_archive_display')->section  = 'strollik5_woocommerce_archive';
            $wp_customizer->get_control('woocommerce_category_archive_display')->priority = 21;

            $wp_customizer->get_control('woocommerce_default_catalog_orderby')->section  = 'strollik5_woocommerce_archive';
        ...
        public function label_tooltip() {
            ...
            echo '<div id="xstreet-woocommerce-cart">' . esc_html__('Add to cart', 'strollik5') . '</div>';
            echo '</div>';
        }
        ...
        public function added_to_cart_template() {
            $text = esc_html__('has been added to your cart', 'strollik5');
            echo <<<HTML
...
HTML;
        ...
        public function widgets_init() {
            register_sidebar(array(
                'name'          => esc_html__('WooCommerce Shop', 'strollik5'),
                'id'            => 'sidebar-woocommerce-shop',
                'description'   => esc_html__('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'strollik5'),
            ...
            register_sidebar(array(
                'name'          => esc_html__('WooCommerce Detail', 'strollik5'),
        ...
        public function render_woocommerce_shop_canvas() {
            if (get_theme_mod('strollik5_woocommerce_archive_filter_position', 'left') === 'none') {
                return;
            }
            $position = get_theme_mod('strollik5_woocommerce_archive_filter_position', 'left');
            if (is_active_sidebar('sidebar-woocommerce-shop-filters')) {
                echo '<div id="opal-canvas-filter" class="opal-canvas-filter ' . esc_attr($position) . '"><span class="filter-close">' . esc_html__('CLOSE', 'strollik5') . '</span><div class="opal-canvas-filter-wrap">';
        ...
        public function render_button_shop_canvas() {
            ...
            if (is_active_sidebar('sidebar-woocommerce-shop-filters')) {
                echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__('Filter', 'strollik5') . '</button>';
            }
        }
        ...
        public function taxonomy_metaboxes() {
            ...
            $cmb_term = new_cmb2_box(array(
                'id'           => 'product_cat',
                'title'        => esc_html__('Product Metabox', 'strollik5'), // Doesn't output for term boxes
            ...
            $cmb_term->add_field(array(
                'name'       => esc_html__('Banner', 'strollik5'),
                'id'         => $prefix . 'banner',
                'type'       => 'file',
                'options'    => array(
        ...
        public function product_video_custom_field() {
            $prefix = 'strollik5_products_';
            $cmb    = new_cmb2_box(array(
                'id'           => $prefix . 'product_video',
                'title'        => esc_html__('Product Video Config', 'strollik5'),
            ...
            $cmb->add_field(array(
                'name' => esc_html__('Product video', 'strollik5'),
                'desc' => esc_html__('Supports video from youtube and vimeo.', 'strollik5'),
                'id'   => $prefix . 'video',
                'type' => 'oembed',
            ));

            $cmb->add_field(array(
                'name'         => esc_html__('Video Thumbnail', 'strollik5'),
        ...
        public function shortcode_loop_end($atts = array()) {
            if (isset($atts['product_layout'])) {
                if ($atts['product_layout'] === 'list') {
                    $this->unset_filter('wc_get_template_part', 'strollik5_woocommerce_change_path_shortcode', 10);
            ...
            if (!empty($atts['image_size'])) {
                $this->unset_filter('woocommerce_product_get_image', array($this, 'set_image_size_list'), 10);
            }
        }
        ...
        public function shortcode_loop_start($atts = array()) {
            if (isset($atts['product_layout'])) {
                if ($atts['product_layout'] === 'list') {
                    add_filter('wc_get_template_part', 'strollik5_woocommerce_change_path_shortcode', 10, 3);
                    if (!empty($atts['image_size'])) {
                        $this->list_size = $atts['image_size'];
                        add_filter('woocommerce_product_get_image', array($this, 'set_image_size_list'), 10, 2);
                    }
                } elseif ($atts['product_layout'] === 'carousel') {
                    echo '<div class="woocommerce-carousel owl-loaded owl-carousel owl-theme" data-settings=\'' . esc_attr($atts['carousel_settings']) . '\'>';
                }
            }
        }

        private function unset_filter( $tag, $function_to_remove, $priority = 10 ) {
            global $wp_filter;
            $unset = 'remove';
            $r = false;
            if ( isset( $wp_filter[ $tag ] ) ) {
                $unset .= '_filter';
                $r = $wp_filter[ $tag ]->$unset( $tag, $function_to_remove, $priority );
            ...
            if (isset($atts['product_layout']) && $atts['product_layout'] != 'grid') {
                ...
                if ($atts['product_layout'] === 'list') {
                    if (!empty($atts['show_category'])) {
                        add_action('strollik5_product_list_before_price', 'strollik5_woocommerce_list_show_category', 15);
                    }

                    if (!empty($atts['show_rating'])) {
                        add_action('strollik5_product_list_before_price', 'strollik5_woocommerce_list_show_rating', 10);
                    }

                    if (!empty($atts['show_except'])) {
                        add_action('strollik5_product_list_after_price', 'strollik5_woocommerce_list_show_excerpt', 15);
                    }
        ...
        public function style_loop_end($atts = array()) {
            if (isset($atts['product_layout']) && $atts['product_layout'] != 'grid') {
                if ($atts['product_layout'] === 'list') {
                    echo '</div>';
                    if (!empty($atts['show_category'])) {
                        remove_action('strollik5_product_list_before_price', 'strollik5_woocommerce_list_show_category', 15);
                    }

                    if (!empty($atts['show_rating'])) {
                        remove_action('strollik5_product_list_before_price', 'strollik5_woocommerce_list_show_rating', 10);
                    }

                    if (!empty($atts['show_except'])) {
                        remove_action('strollik5_product_list_after_price', 'strollik5_woocommerce_list_show_excerpt', 15);
                    }
                }
            }
        }
        ...
        public function body_class($classes) {
            ...
            if (strollik5_is_product_archive()) {
                ...
                if (get_theme_mod('strollik5_woocommerce_archive_product_width', 0)) {
                    $classes[] = 'strollik5_woocommerce_archive_product_style_full';
                }
            } else {
                if (is_product()) {
                    $classes = array_diff($classes, array(
                    ...
                    $classes[] = 'woocommerce-single-style-' . get_theme_mod('strollik5_woocommerce_single_product_style', '1');
                    if (get_theme_mod('strollik5_woocommerce_single_product_width', 0)) {
                        $classes[] = 'strollik5_woocommerce_single_product_style_full';
                    }

                }
            }

            $classes[] = 'product-style-' . get_theme_mod('strollik5_woocommerce_product_style', 1);

            if (get_theme_mod('strollik5_woocommerce_product_boxshadow_custom_enable', 0)) {
        ...
        public function set_sidebar($name) {
            if (strollik5_is_product_archive() && is_active_sidebar('sidebar-woocommerce-shop')) {
        ...
        public function related_products_args($args) {
            $args = apply_filters('strollik5_related_products_args', array(
                'posts_per_page' => get_theme_mod('strollik5_woocommerce_single_related_number', 3),
                'columns'        => get_theme_mod('strollik5_woocommerce_single_related_columns', 3),
            ));
        ...
        public function thumbnail_columns() {
            $columns = get_theme_mod('strollik5_woocommerce_product_thumbnail_columns', 3);

            return intval(apply_filters('strollik5_product_thumbnail_columns', $columns));
        }
        ...
        public function products_per_page() {
            $number = get_theme_mod('strollik5_woocommerce_archive_number', 12);

            return intval(apply_filters('strollik5_products_per_page', $number));
        }
        ...
        public function customizer_buttons($buttons) {
            $buttons = wp_parse_args($buttons, array(
                '.single-product #content'             => array(
                    array(
                        'id'   => 'strollik5_woocommerce_single',
                        'icon' => 'default',
                        'type' => 'section',
                    ),
                ),
                '.archive.woocommerce-page #content'   => array(
                    array(
                        'id'   => 'strollik5_woocommerce_archive',
                        'icon' => 'default',
                        'type' => 'section',
                    ),
                ),
                '.woocommerce-pagination'              => array(
                    array(
                        'id'      => 'strollik5_layout_pagination_style',
                        'icon'    => 'default',
                        'type'    => 'control',
                ...
                '.single-product .flex-control-thumbs' => array(
                    array(
                        'id'      => 'strollik5_woocommerce_product_thumbnail_columns',
                        'icon'    => 'default',
                        'type'    => 'control',
                ...
                '.single-product .related'             => array(
                    array(
                        'id'      => 'strollik5_woocommerce_single_related_columns',
                        'icon'    => 'default',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
                ...
                '.single-product .upsells'             => array(
                    array(
                        'id'      => 'strollik5_woocommerce_single_upsale_columns',
                        'icon'    => 'default',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
                ...
                '.products .type-product'              => array(
                    array(
                        'id'      => 'strollik5_woocommerce_product_hover',
                        'icon'    => 'default',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
                ),
                '#xstreet-accordion-container'         => array(
                    array(
                        'id'      => 'strollik5_woocommerce_single_product_tab_style',
                        'icon'    => 'layout',
                        'type'    => 'control',
                        'trigger' => 'select|focus',
                    ),
        ...
        public function custom_comment_form($comment_form) {
            ...
            $comment_form['fields']        = array(
                'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__('Name', 'strollik5') . '&nbsp;<span class="required">*</span></label> ' .
                            '<input id="author" name="author" type="text" placeholder="' . esc_attr__("Name", "strollik5") . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" required /></p>',
                'email'  => '<p class="comment-form-email"><label for="email">' . esc_html__('Email', 'strollik5') . '&nbsp;<span class="required">*</span></label> ' .
                            '<input id="email" name="email" type="email" placeholder="' . esc_attr__("Email", "strollik5") . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" required /></p>',
            );
            $comment_form['comment_field'] = '<p class="comment-form-comment"><label for="comment">' . esc_html__('Your review', 'strollik5') . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" required placeholder="' . esc_attr__("Your review", "strollik5") . '"></textarea></p>';
            return $comment_form;
        }
...
strollik5_WooCommerce::getInstance();