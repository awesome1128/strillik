...
class strollik5_setup_theme {
    function __construct() {
        ...
        add_filter('comment_form_default_fields', array($this, 'strollik5_comment_fields'));
        add_filter('the_content_more_link', array($this, 'strollik5_morelink'), 10, 2);
    }
    ...
    public function add_scripts() {
        ...
        if (!get_theme_mod('osf_dev_mode', false)) {
            ...
            if (!class_exists('OSF_Scripts')) {
                wp_enqueue_style('strollik-colors', get_theme_file_uri('assets/css/color.css'), array('strollik-style'));
                wp_style_add_data('strollik-colors', 'rtl', 'replace');
            }
        }

        if (strollik5_is_elementor_activated()) {
            $deps[] = 'elementor-frontend';
        }
        // Add custom fonts, used in the main stylesheet.
        wp_enqueue_style('strollik-style', get_parent_theme_file_uri('style.css'), $deps);
        wp_style_add_data('strollik-style', 'rtl', 'replace');

        if (!class_exists('OSF_Scripts')) {
            $google_fonts_url = '//fonts.googleapis.com/css?family=Rubik:300,400,500,700';
            if ($google_fonts_url) {
        ...
        // Sticky Sidebar
        wp_enqueue_script('strollik-theme-sticky-layout-js', get_theme_file_uri('/assets/js/sticky-layout.js'), array(
            'jquery',
            'wp-util'
        ), false, true);
        ...
        if (has_nav_menu('top')) {
            ...
            $opal_l10n['expand']   = esc_html__('Expand child menu', 'strollik5');
            $opal_l10n['collapse'] = esc_html__('Collapse child menu', 'strollik5');
            $opal_l10n['icon']     = '<i class="fa fa-angle-down"></i>';
        }

        wp_localize_script('strollik-theme-js', 'strollik5JS', $opal_l10n);
    ...
    public function widgets_init() {
        ...
        register_sidebar(array(
            'name'          => esc_html__('Footer 1', 'strollik5'),
            'id'            => 'footer-1',
            'description'   => esc_html__('Add widgets here to appear in your footer.', 'strollik5'),
        ...
        register_sidebar(array(
            'name'          => esc_html__('Footer 2', 'strollik5'),
            'id'            => 'footer-2',
            'description'   => esc_html__('Add widgets here to appear in your footer.', 'strollik5'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
        ...
        register_sidebar(array(
            'name'          => esc_html__('Footer 3', 'strollik5'),
            'id'            => 'footer-3',
            'description'   => esc_html__('Add widgets here to appear in your footer.', 'strollik5'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
    ...
    public function excerpt_more($link) {
        ...
        $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>',
            ...
            sprintf(__('Read More <span class="screen-reader-text"> "%s"</span>', 'strollik5'), get_the_title(get_the_ID()))
        );
    ...
    public function setup() {
        load_theme_textdomain('strollik5', get_template_directory() . '/languages');
        ...
        register_nav_menus(array(
            'top' => esc_html__('Top Menu', 'strollik5'),
        ));
        ...
        add_image_size('strollik-featured-image-large', 500, 300, true);
        add_image_size('strollik-gallery-image', 700, 9999, false);
        add_image_size('strollik-product-thumbnail', 220, 280, true);
    ...
    public function strollik5_comment_fields($fields) {
        $commenter = wp_get_current_commenter();
        $req       = get_option('require_name_email');
        $html_req  = ($req ? " required='required'" : '');

        $fields = array(
            'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__('Name', 'strollik5') . ($req ? ' <span class="required">*</span>' : '') . '</label> ' .
                        '<input id="author" name="author" type="text" placeholder="' . esc_attr__("Name", "strollik5") . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" maxlength="245"' . $html_req . ' /></p>',
            'email'  => '<p class="comment-form-email"><label for="email">' . esc_html__('Email', 'strollik5') . ($req ? ' <span class="required">*</span>' : '') . '</label> ' .
                        '<input id="email" name="email" type="email"  placeholder="' . esc_attr__("Email", "strollik5") . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" maxlength="100" aria-describedby="email-notes"' . $html_req . ' /></p>',
            'url'    => '<p class="comment-form-url"><label for="url">' . esc_html__('Website', 'strollik5') . '</label> ' .
                        '<input id="url" name="url" type="url"  placeholder="' . esc_attr__("Website", "strollik5") . '" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" maxlength="200" /></p>',
        );

        return $fields;
    }

    public function strollik5_morelink($more_link, $more_link_text) {
        return '<span class="more-link-wrap">' . wp_kses_post($more_link) . '</span>';
    }
}

return new strollik5_setup_theme();