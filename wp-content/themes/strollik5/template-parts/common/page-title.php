...
if (!strollik5_page_enable_breadcrumb()) {
    return;
}
...
$otf_home = esc_html__('Home', 'strollik5');
$otf_blog = esc_html__('Blog', 'strollik5');
$otf_shop = esc_html__('Shop', 'strollik5');
$otf_title = '';
...
if (!function_exists('bcn_display')) {
    ...
    if (!is_front_page()) {
        ...
        if (is_category()) {
            ...
            $otf_title = esc_html__('Day', 'strollik5');
            // Day archive

            // Year link
            $otf_output .= '<li class="list-inline-item item"><a href="' . esc_url(get_year_link(get_the_time('Y'))) . '" title="' . esc_attr(get_the_time('Y')) . '">' . get_the_time('Y') . esc_html__(' Archives', 'strollik5') . '</a></li>';
            $otf_output .= '<li class="list-inline-item separator"> ' . wp_kses_post($otf_sep) . ' </li>';

            // Month link
            $otf_output .= '<li class="list-inline-item item"><a href="' . esc_url(get_month_link(get_the_time('Y'), get_the_time('m'))) . '" title="' . esc_attr(get_the_time('M')) . '">' . get_the_time('M') . esc_html__(' Archives', 'strollik5') . '</a></li>';
            $otf_output .= '<li class="list-inline-item separator"> ' . wp_kses_post($otf_sep) . ' </li>';

            // Day display
            $otf_output .= '<li class="list-inline-item item current"> ' . get_the_time('jS') . ' ' . get_the_time('M') . esc_html__(' Archives', 'strollik5') . '</li>';

        } elseif (is_month()) {
            $otf_title = get_the_time('M') . esc_html__(' Archives', 'strollik5');
            // Month Archive
            ...
            $otf_title = get_the_time('Y') . esc_html__('Archives', 'strollik5');
            ...
            if (!empty($author->ID)) {
                ...
                $otf_title = esc_html__('Author: ', 'strollik5') . $otf_userdata->display_name;
                // Get the author information
                $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Author: ', 'strollik5') . '<a href="' . get_author_posts_url($otf_userdata->ID, $otf_userdata->nice_name) . '">' . esc_html($otf_userdata->display_name) . '</a></li>';
            }

        } elseif (get_query_var('paged')) {
            // Paginated archives
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Page', 'strollik5') . ' ' . get_query_var('paged', '1') . '</li>';

        } elseif (is_search()) {
            $otf_title = esc_html__('Search', 'strollik5');
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Keyword: ', 'strollik5') . get_search_query() . '</li>';

        } elseif (is_404()) {
            $otf_title = esc_html__('Error 404', 'strollik5');
            $otf_output .= '<li class="list-inline-item item current">' . wp_kses_post($otf_title) . '</li>';
        }
    }
    $otf_output .= '</ul>';
    ...
    if (is_home()) {
        ...
        $otf_title = get_the_time('D') . esc_html__(' Archives', 'strollik5');
    } elseif (is_month()) {
        $otf_title = get_the_time('M') . esc_html__(' Archives', 'strollik5');
    } elseif (is_year()) {
        $otf_title = get_the_time('Y') . esc_html__('Archives', 'strollik5');
        ...
        global $author;
        if (!empty($author->ID)) {
            $otf_title = esc_html__('Author', 'strollik5');
        }
